...
export class AutoCADClientConnection {
  host: string;
  port: number;
  socket: net.Socket;
  ...
  private setupSocketListeners(): void {
    ...
    this.socket.on("error", (error) => {
      console.error("AutoCADClientConnection error:", error);
      this.isConnected = false;
    });
  }
      ...
      try {
        ...
        this.responseCallbacks.set(requestId, (responseData) => {
          try {
            const response = JSON.parse(responseData);
            if (response.error) {
              reject(new Error(response.error.message || "Unknown error from AutoCAD"));
            } else {
