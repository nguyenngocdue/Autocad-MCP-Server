...
import { setAutoCADHttpUrl, getAutoCADHttpUrl } from "./utils/AutoCADHttpClient.js";
...
async function startHttp() {
  ...
  // UI page to set AUTOCAD_HTTP_URL
  app.get("/connect", (_req, res) => {
    const currentUrl = getAutoCADHttpUrl() || "";
    res.setHeader("Content-Type", "text/html");
  ...
  // Debug endpoint — show AutoCAD connection config and test TCP
  app.get("/debug", async (_req, res) => {
    const host = process.env.AUTOCAD_HOST || "localhost";
    const port = process.env.AUTOCAD_PORT || "(scan 8180-8199)";
    const portNum = process.env.AUTOCAD_PORT ? parseInt(process.env.AUTOCAD_PORT, 10) : 8180;
  ...
  // Debug send — test full command/response cycle with AutoCAD
  ...
  // Set AUTOCAD_HTTP_URL at runtime — không cần redeploy khi Cloudflare URL thay đổi
  app.post("/set-autocad-url", (req, res) => {
    const { url } = req.body as { url?: string };
    if (!url || typeof url !== "string" || !url.startsWith("http")) {
      res.status(400).json({ error: "Missing or invalid url" });
      return;
    }
    setAutoCADHttpUrl(url.trim());
    console.error(`[Config] AUTOCAD_HTTP_URL updated to: ${url.trim()}`);
    res.json({ ok: true, url: url.trim() });
  });

  // Get current AUTOCAD_HTTP_URL
  app.get("/autocad-url", (_req, res) => {
    res.json({ url: getAutoCADHttpUrl() || null });
  });
  ...
  app.listen(port, () => {
    console.error(`AutoCAD MCP Server (HTTP/Streamable) listening on port ${port}`);
  });
}
...
async function startStdio() {
  ...
  console.error("AutoCAD MCP Server started (stdio).");
}
...
main().catch((error) => {
  console.error("Error starting AutoCAD MCP Server:", error);
  process.exit(1);
});
